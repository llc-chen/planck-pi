...
{
    /* GPIOE配置 */
    *((volatile size_t*)gpioe_cfg0) &= ~(7<<16); //清除配置寄存器
    *((volatile size_t*)gpioe_cfg0) |= (1<<16);  //配置GPIOE4为输出模式
    *((volatile size_t*)gpioe_pul0) &= ~(3<<16); //清除上/下拉寄存器
    *((volatile size_t*)gpioe_pul0) |= (1<<12);  //配置GPIOE4为上拉模式
...
{
    int ret;
    size_t status = ((*((volatile size_t*)gpioe_data))>>4)&0x01;//获取GPIOE12状态
...
static int led_write(struct file *filp, const char __user *buff, size_t count, loff_t *offp)
{
    ...
    *((volatile size_t*)gpioe_data) &= ~(1<<4) ;//清除GPIOE4状态
    if(status)
        *((volatile size_t*)gpioe_data) |= (1<<4);//设置GPIOE4状态1
    return 0;
}
...
static int __init led_init(void)   
{
    ...
    ret = alloc_chrdev_region(&led_dev_num,0,1,"led_usr");  //动态申请一个设备号
    if(ret !=0)
    {
    ...
    led0 = device_create(led_class,NULL,led_dev_num,NULL,"led_usr");//创建一个设备名为led0
    if(IS_ERR(led0))
    {
